...
function makeRenderPct(valueKey: 'soLuong' | 'giaTri', total: number) {
  return function renderPct(props: Record<string, any>) {
...
/**
 * Báo cáo tồn theo mốc thời gian (donut số lượng + giá trị, bảng tổng) — dùng chung cho
 * nguyên liệu / thành phẩm. SP đều trả về cột `ten_StockTime`, `mau`, `soLuong`, `giaTri`.
 */
export default function StockTimeView({ rows, loading = false }: StockTimeViewProps) {
  const data = useMemo<ChartRow[]>(
    () =>
      ((rows ?? []) as StockTimeRow[]).map((row) => ({
        ...row,
